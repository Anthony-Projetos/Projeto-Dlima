...
class LabelPrintingFrontendTests(TestCase):
    def test_js_de_etiquetas_usa_tspl_raw_60x40(self):
        ...
        self.assertIn('SIZE 40 mm,60 mm', source)
        ...
        self.assertIn('buildTsplBar(0, 96, LABEL_WIDTH_DOTS, 3)', source)
        self.assertIn('buildTsplBar(0, 238, LABEL_WIDTH_DOTS, 3)', source)
        self.assertIn('buildTsplBar(0, 330, LABEL_WIDTH_DOTS, 3)', source)
        self.assertIn('buildTsplBar(0, 420, LABEL_WIDTH_DOTS, 3)', source)
        self.assertIn("buildCenteredTsplText(20, \"D'LIMA\"", source)
        self.assertIn("buildCenteredTsplText(66, 'S T O R E'", source)
        self.assertIn('buildCenteredTsplText(110, productLines[0]', source)
        self.assertIn('buildCenteredTsplText(174, productLines[1]', source)
        self.assertIn("buildTsplText(26, 274, 'TAM:'", source)
        self.assertIn('buildCenteredTsplText(260, tamanho', source)
        self.assertIn('buildCenteredTsplText(350, preco', source)
        self.assertIn('buildCenteredTsplText(434, footerText', source)
        ...
        self.assertNotIn('buildRotatedTsplText', source)
        self.assertNotIn('buildRotatedCenteredTsplText', source)
        self.assertNotIn('buildRotatedTsplBar', source)
        self.assertNotIn('rotation || 270', source)
        self.assertNotIn('rotate(-90deg)', css := Path(settings.BASE_DIR / 'static/css/style.css').read_text(encoding='utf-8'))
