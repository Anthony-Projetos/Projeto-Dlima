...
class LabelPrintingFrontendTests(TestCase):
    ...
    def test_js_de_etiquetas_oferece_html_qz_60x40(self):
        source = Path(settings.BASE_DIR / 'static/js/vendas/registrar_venda.js').read_text(encoding='utf-8')
        styles = Path(settings.BASE_DIR / 'static/css/style.css').read_text(encoding='utf-8')
        template = Path(settings.BASE_DIR / 'templates/vendas/registrar_venda.html').read_text(encoding='utf-8')
        ...
        self.assertIn('background: white;', source)
        self.assertIn('color: black;', source)
        self.assertIn('D&#39;lima', source)
        self.assertIn('<header class="label-top">', source)
        self.assertIn('class="label-product-name"', source)
        self.assertIn('REF.', source)
        self.assertIn('class="label-size"', source)
        self.assertIn('VALOR', source)
        self.assertIn('&quot;Estilo não é seguir regras,', source)
        self.assertIn('é criar o seu caminho.&quot;', source)
        self.assertIn('DLIMA STORE', source)
        self.assertIn('.label-preview .label-product-name', styles)
        self.assertIn('.label-preview .label-value strong', styles)
        self.assertNotIn('label-product-title', source)
        self.assertNotIn('label-size-title', source)
