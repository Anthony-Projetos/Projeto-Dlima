...
class LabelPrintingFrontendTests(TestCase):
    def test_js_de_etiquetas_usa_tspl_raw_60x40(self):
        ...
        self.assertIn('function getProductTsplOptions(value)', source)
        self.assertIn('buildTsplBar(92, 24, 2, 272)', source)
        self.assertIn('buildTsplBar(190, 24, 2, 272)', source)
        self.assertIn('buildTsplBar(320, 24, 2, 272)', source)
        self.assertIn('buildTsplBar(390, 24, 2, 272)', source)
        self.assertIn('buildTsplBar(430, 24, 2, 272)', source)
        self.assertIn('buildTsplBar(204, 154, 106, 2)', source)
        self.assertIn("buildTsplText(34, 244, \"D'lima\"", source)
        self.assertIn("buildTsplText(66, 178, 'store'", source)
        self.assertIn('buildTsplText(112, 292, productLines[0]', source)
        self.assertIn('buildTsplText(150, 292, productLines[1]', source)
        self.assertIn("buildTsplText(204, 132, 'VALOR'", source)
        self.assertIn('buildTsplText(244, 166, preco', source)
        self.assertIn("buildTsplText(204, 292, 'REF.'", source)
        self.assertIn('buildTsplText(232, 292, referencia', source)
        self.assertIn('buildTsplText(276, 292, tamanho', source)
        self.assertIn('D L I M A  S T O R E', source)
        self.assertIn('label-reference-product', source)
        self.assertIn('label-reference-price', source)
