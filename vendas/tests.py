...
class LabelPrintingFrontendTests(TestCase):
    def test_js_de_etiquetas_usa_qz_pixel_html_60x40_e_40x60(self):
        ...
        self.assertIn("'60x40': {", source)
        self.assertIn("width: 60", source)
        self.assertIn("height: 40", source)
        self.assertIn("orientation: 'landscape'", source)
        self.assertIn("'40x60': {", source)
        self.assertIn("width: 40", source)
        self.assertIn("height: 60", source)
        self.assertIn("orientation: 'portrait'", source)
        ...
        self.assertIn('@page { size: ${size.width}mm ${size.height}mm; margin: 0; }', source)
        self.assertIn('width: ${size.width}mm;', source)
        self.assertIn('height: ${size.height}mm;', source)
        ...
        self.assertIn('border: 1px solid black;', source)
        self.assertIn('label--garment', source)
        self.assertIn('label-brand', source)
        self.assertIn('Dlima Store', source)
        self.assertIn('label-product-name', source)
        self.assertIn('label-details-grid', source)
        self.assertIn('label-detail-title', source)
        self.assertIn('label-price-panel', source)
        self.assertIn('label-price-value', source)
        self.assertIn('label-footer', source)
        self.assertIn('buildLabelPrintData', source)
        self.assertIn('buildLabelPrintCss(state)', source)
        self.assertIn('await window.qz.print(config, etiquetas);', source)
        self.assertIn('value="60x40"', template)
        self.assertIn('60x40 landscape', template)
        self.assertIn('value="40x60"', template)
        self.assertIn('40x60 portrait teste', template)
